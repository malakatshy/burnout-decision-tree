...
exports.gini = gini;
exports.findBestSplit = findBestSplit;
exports.buildTree = buildTree;
exports.predict = predict;
exports.predictWithPath = predictWithPath;
...
function partition(rows, feature, threshold) {
    const left = [];
    const right = [];
    for (const row of rows) {
        const value = row[feature];
        const goesLeft = typeof value === "number" ? value < threshold : value === threshold;
        if (goesLeft)
            left.push(row);
        else
            right.push(row);
...
function findBestSplit(rows) {
    ...
    const features = ["sleep", "meetings", "weekends", "stress"];
    let best = null;
...
function predict(node, sample) {
    ...
    const goesLeft = typeof value === "number" // number ?
        ? value < node.threshold // numeric: left = value < threshold
...
function predictWithPath(node, sample, nodeId = "root") {
    if (node.kind === "leaf") {
        return {
    ...
    const value = sample[node.feature];
    const goesLeft = typeof value === "number"
